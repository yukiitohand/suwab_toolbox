function [x,z,C,d,rhov,res_p,res_d] = huwacb_gadmm2_advresblc(A,y,wv,varargin)
...
%         tic; res_p = sqrt(ones1NL* st.^2 * onesNy1);toc;
        res_p = norm(st,'fro');
        res_p2 = norm(ddhat,'fro');
%         res_d = sqrt((rhov'.^2)*ss0.^2 * onesNy1);
        res_d = norm(rhov.*(ss0),'fro');
        res_d2 = norm(rhov.*(tt0),'fro');
%         res_p = norm(s-t,'fro');
%         % dual feasibility
%         res_d = rhov*(norm((s-s0),'fro'));
        if  verbose
            fprintf(' k = % 4d, res_p = %10f, res_p2 = %10f, res_d = %10f, res_d2 = %10f\n',k,res_p,res_p2,res_d,res_d2)
        end
     end
...
%         tic; res_pv = sqrt(sum((s-t).^2,2)); toc;
        % dual feasibility
        res_dv = rhov.*sqrt(abs(ss0.*tt0) * onesNy1);
%         tic; res_dv = rhov.*vnorms(s-s0,2); toc;

        rhov_new = res_pv./res_dv;
        rhov_new(rhov_new>1) = 1;
        rhov_new(isnan(rhov_new)) = 0.0001;
        rhov_new(rhov_new==0) = 0.00001;

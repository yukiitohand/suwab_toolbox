function [x,z,C,r,d,rho,res_p,res_d] = huwacbl1_admm3_indrho(A,y,wv,varargin)
...
% via alternating direction method of multipliers (ADMM). Different
% spectral parameters rho are assigned for each column.
%
%  Inputs
...
K = [eye(NL);T];
KtKinv = (T'*T+eye(NL)) \ eye(NL);
KtKinvKt = KtKinv*K';
ybar = [zeros(NL,Ny);y];
Kty = T'*y;
KtKinvKty = KtKinv * Kty;
% projection operator
c1 = zeros([N+2*L,Ny]);
...
%     end
    ...
    if mod(k,10) == 0
        % primal feasibility
        res_pv = vnorms(Ks_minus_ybar-t,1);
        % dual feasibility

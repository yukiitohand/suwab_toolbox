function [x,z,C,rhov] = huwacb_dal_gadmm(A,y,wv,varargin)
% [x,z,res_p,res_d] = huwacb_dal_gadmm(A,y,wv,varargin)
% L1-constrained hyperspectral unmixing with adaptive concave background (HUWACB) via 
% dual augmented Lagrangian (DAL) with a generalized ADMM using residual balancing
%
%  Inputs
...
%     'VERBOSE' : {'yes', 'no'}
%     'RHOV'     : a vector of spectral penalty parameters
%                  (default) 0.01*ones(N+L,1)
...
%   solved via a generalized ADMM. Its augmented Lagrangian is defined 
%   L = f*(aa) + g*(vv) + d.T(vv-Tt*aa) + 0.5||P(vv-Tt*a)||_2^2
%   where P is a diagnoal matrix whose diagonal elements are spectral
%   penalty parameters.
%   Refer 
% 
...
Q = (eye(L) + bsxfun(@times,T,rhov')*T.') \ eye(L);
...
%     end
    if mod(k,10) == 0 || k==1
        ...
        % dual feasibility
        res_d = norm(T*(rhov.*(vv-vv0)),'fro');
        res_d2 = norm(rhov.*sqrt((T'*(aa-aa0)).^2*onesNy1),'fro');
%         res_d = norm(rhov'*sqrt(abs(ss0.*tt0)),'fro');
        if  verbose
            fprintf(' k = %f, res_p = %e, res_d = %e, res_d2 = %e\n',k,res_p,res_d,res_d2);
        end
     end
...
%     if (mod(k,20) == 0 && k<101) || (mod(k,100)==0 && k>101)
        % primal feasibility
        res_pv = sqrt((T'*aa-vv).^2 * onesNy1);
        % dual feasibility
%         res_dv = rhov.*sqrt((tt0).^2 * onesNy1);
        res_dv = rhov.*sqrt((T'*(aa-aa0)).^2*onesNy1);

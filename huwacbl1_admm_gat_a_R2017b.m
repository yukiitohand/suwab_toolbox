function [x,z,C,r,d,rho,Rhov,res_p,res_d,cost_val] = huwacbl1_admm_gat_a_R2017b(A,y,wv,varargin)
% [x,z,C,r,d,rho,Rhov,res_p,res_d] = huwacbl1_admm_gat_a_R2017b(A,y,wv,varargin)
...
%
% This is the version compatible to R2017b or earlier. (relating to new 
% input patterns of functions "sum" and "mean".)
% diverged from huwacbl1_admm_gat_a.m of the version fbb2c2e 
%
%  Inputs
...
while (k <= maxiter) && ((abs(res_p) > tol_p) || (abs(res_d) > tol_d)) 
    if isdebug
        cost_val = sum(sum(abs(y-A*t(1:N,:)-C*t(N+1:N+L,:))./tau,1),2)+sum(sum(abs(lambda_a.*t(1:N,:)),1),2);
...
if Aisempty
    cost_val = sum(sum(abs(y-C*z)./tau,1),2);
else
    cost_val = sum(sum(abs(y-A*x-C*z)./tau,1),2)+sum(sum(abs(lambda_a.*x),1),2);
end

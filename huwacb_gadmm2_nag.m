function [x,z,C,d,rhov,res_p,res_d] = huwacb_gadmm2_nag(A,y,wv,varargin)
% [x,z,res_p,res_d] = huwacb_gadmm2(A,y,wv,varargin)
% hyperspectral unmixing with adaptive concave background (HUWACB) with 
% sparsity constraint via a generalized alternating direction method of 
% multipliers (GADMM)
% 
% Nesterov's accelerated gradient (NAG) is implmented together with residual
% balancding.
%  
%
%  Inputs
...
%
%   For the adoption of NAG to ADMM, see
%       Goldstein, T., O?Donoghue, B., Setzer, S., Baraniuk, R., 2014. Fast
%       alternating direction optimization methods. SIAM J. Imaging Sci. 7,
%       1588?1623. https://doi.org/10.1137/120896219
%
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%     t = 0.6*t +0.4*s; % under-relaxation
    ...
    ck = norm(dhat-d,'fro').^2 + sum(rhov.^2'*(shat-s).^2);
    if k>2
        if ck < eta*ckm1
            akp1 = (1 + sqrt(1+4*ak.^2)) * 0.5;
            stp = (ak-1)/akp1;
            dhat = d + stp*(d-d0);
            shat = s + stp*(s-s0);
            ak = akp1;
    ...
    if mod(k,10) == 0 || k==1
        % primal feasibility
        st = s-t; ss0 = s-s0;
%         tic; res_p = sqrt(ones1NL* st.^2 * onesNy1);toc;
        res_p = norm(st,'fro');
%         res_d = sqrt((rhov'.^2)*ss0.^2 * onesNy1);
        res_d = norm(rhov.*(ss0),'fro');
%         res_p = norm(s-t,'fro');
%         % dual feasibility
%         res_d = rhov*(norm((s-s0),'fro'));
        if  verbose
...
%         tic; res_pv = sqrt((s-t).^2 * ones1Ny); toc;
%         res_pv = vnorms(s-t,2);
%         tic; res_pv = sqrt(sum((s-t).^2,2)); toc;
        % dual feasibility
        res_dv = rhov.*sqrt((ss0).^2 * onesNy1);
%         tic; res_dv = rhov.*vnorms(s-s0,2); toc;
        
%         if update_rho_active %&& k < maxiter
            % update rho

function [x,z,C,rho,res_p,res_d] = huwacbl1_admm2_1(A,y,wv,varargin)
...
%     end
    ...
    if mod(k,10) == 0
        % primal feasibility
        res_pv = vnorms(x-u,1) + vnorms(z-v,1) + vnorms(A*x+C*z-r-y,1);
        % dual feasibility
        res_dv = rho.*(vnorms((u-u0),1)+vnorms(v-v0,1)+vnorms(r-r0,1));

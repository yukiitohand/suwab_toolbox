function [ x,r,d,rho,Rhov,res_pv,res_dv,cost_val,Kcond ] = lad_admm_gat_b_batch_R2017b( A,y,varargin )
%  [ x,r,d,rho,Rhov,res_pv,res_dv,cost_val,Kcond ]
%   = lad_admm_gat_b_batch_R2017b( A,y,varargin )
...
% diverged from lad_admm_gat_b_batch.m of the version 97df9ca
%
% Input Parameters
...
while (k <= maxiter) && ((abs(res_p) > tol_p) || (abs(res_d) > tol_d))
    if isdebug
        cost_val = sum(sum(sum(abs(A*t(1:N,:)-y),1),2),3);

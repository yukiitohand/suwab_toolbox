function [x,z,C,r,d,rho,Rhov,res_p,res_d] = huwacbl1_gadmm_a_v2_gpu(A,y,wv,varargin)
...
%toc;
% if ~existsOnGPU(A), A = gpuArray(A); end
% if ~existsOnGPU(y), y = gpuArray(y); end
% if ~existsOnGPU(wv), wv = gpuArray(wv); end
A = gpuArray(A); y = gpuArray(y); wv = gpuArray(wv);
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
c1(N+L+1:N+L*2,:) = ones([L,1],'gpuArray')./tau*tau1;
c1rho = c1./rho./Rhov;

c2 = zeros([N+2*L,1],'gpuArray');

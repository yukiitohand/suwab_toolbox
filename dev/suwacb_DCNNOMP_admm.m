function [ XA,XC,res,notConvergeFlag,Ctmp ] = suwacb_DCNNOMP_admm( x,A,Y,R,K,varargin )
...
for n=1:numblocks %numblocks
    ...
    if root
        ...
        [ XAtmp2,XCtmp2,r_mintmp2,notConvergeFlagtmp2 ]...
            = suwacb_DCNNOMP_admm( x,A,Yn(:,notConvergeFlagtmp),Rtmp(:,notConvergeFlagtmp),[],varargin{:});
        ...
        K_bar = setdiff(1:p,K);
        [XAAKbar,XCAKbar,Ctmp,dtmp,rho] = huwacb_admm2_stflip(A(:,K),A(:,K_bar),x,'tol',1e-6,'maxiter',1000);
        % rAKbar = A(:,K_bar) - A(:,K)*XAAKbar - Ctmp*XCAKbar;
        % [ XAAKbar,XCAKbar,rAKbar ] = huwacb(x,A(:,K),A(:,K_bar));
        if isempty(K)
            RAKbar = A(:,K_bar) - Ctmp*XCAKbar;
        else
            RAKbar = A(:,K_bar) - A(:,K)*XAAKbar - Ctmp*XCAKbar;
        end
        ...
        for k = u_idxes
            ...
            [XAtmpk,XCtmpk,Ctmp,dtmp,rho] = huwacb_admm2_stflip(A(:,[K k]),Yn(:,k_idxes),x,'tol',1e-6,'maxiter',1000);
            rtmpk = vnorms(Yn(:,k_idxes) - A(:,[K k])*XAtmpk - Ctmp*XCtmpk,1,2);

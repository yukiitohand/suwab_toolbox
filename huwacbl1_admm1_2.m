function [x,b,C,res_p,res_d] = huwacbl1_admm1_2(A,y,wv,varargin)
...
PT_ort_bu = Cinv * PT_ort(N+1:N+L,1:N);
PT_ort_bv = Cinv * PT_ort(N+1:N+L,N+1:N+L);
PT_ort_bp = Cinv * PT_ort(N+1:N+L,N+L+1:N+L*2);
...
%     end
    ...
    if mod(k,10) == 0
        ...
        res_p = norm(x-u,'fro') + norm(C*b-v,'fro') + norm(r-p,'fro');
        % dual feasibility
        res_d = rho*(norm((u-u0),'fro')) + rho*(norm(Cinv*(v-v0),'fro')) + rho*(norm((p-p0),'fro'));
        if  verbose
            fprintf(' k = %f, res_p = %f, res_d = %f\n',k,res_p,res_d)
        end

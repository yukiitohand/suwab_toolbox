function [x,z,C] = huwacb_fista(A,y,wv,varargin)
% [x,z,res_p,res_d] = huwacb_fista(A,y,wv,varargin)
...
%                                           |_  -inf _|
%
%   This problem is solved via FISTA.
%   Reference
...
lambda_a_v_L = lambda_a_v ./ L;
kappa = zeros(NL,1);
kappa(1:N) = lambda_a_v_L;
kappa_ = zeros(NL,1);
...
TtT = T.'*T;
grad = @(x1) TtT*x1 - ayy;
proj = @(x1) max(x1-kappa,kappa2);
F = @(x1) 0.5*norm(T*x1-y,'fro')^2 + norm1(kappa_.*x1);
...
cost_old = F(y_old);
while  k <= maxiter
    x_new = proj(y_old-Linv*grad(y_old));
